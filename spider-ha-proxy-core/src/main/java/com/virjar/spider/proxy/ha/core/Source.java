...
import static com.virjar.spider.proxy.ha.Configs.openPortSet;
...
//            .build();
    ...
    void handleUpstreamResource(IpAndPort ipAndPort) {
        ...
        // 当可用upstream数量大于2倍的端口映射数量 将代理ip缓存 不创建连接
    ...
    private void parseConfig() {
        ...
        for (Integer port : needBindPort) {
            if (!openPortSet.add(Configs.listenIp + ":" + port)) {
    ...
    public void destroy() {
        Set<Integer> ports = mapping.keySet();
        clearOpenPort();
        for (Integer port : ports) {
            HaProxyMapping haProxyMapping = mapping.get(port);
            haProxyMapping.destroy();
        }

        for (Upstream upstream : availableUpstream) {
            upstream.setBlack(true);
            upstream.become(UpstreamStat.DESTROYED);
        }
    }

    private void clearOpenPort() {
        availableIpAndPort.clear();
        needBindPort.clear();
        Iterable<String> pairs = Splitter.on(":").split(mappingSpace);
        for (String pair : pairs) {
            if (pair.contains("-")) {
                ...
                int end = Integer.parseInt(endStr);
                for (int i = start; i <= end; i++) {
                    Configs.openPortSet.remove(Configs.listenIp + ":" + i);
                }
            }
        }
    }
